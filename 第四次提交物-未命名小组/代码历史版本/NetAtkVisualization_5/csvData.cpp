...
{
    return cmpArray(s_Device_IP, data.s_Device_IP, 4) && cmpArray(d_Device_IP, data.d_Device_IP, 4) && s_Device_port == data.s_Device_port &&
           d_Device_port == data.d_Device_port && protocol == data.protocol && length == data.length;
}

bool CsvData::operator ==(Line line)
{
    return cmpArray(s_Device_IP, line.s_Device_IP, 4) && cmpArray(d_Device_IP, line.d_Device_IP, 4);
}
