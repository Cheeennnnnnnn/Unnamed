...
DataReader_csv::DataReader_csv(QString _filePath, float _rate, int _cycle, int _validTime) :
    ...
    csvProcessor(new CsvProcessor(keepReading)),
    currentRate(_rate),
    cycle(_cycle),
...
void DataReader_csv::run()
{
    while(keepReading)
    {
        static clock_t start = clock(), finish;
        long long timeUsed;
        ...
        csvProcessor ->clearData(timeIndex - validTime);

        finish = clock();
        timeUsed = finish - start;

        if(timeUsed < cycle / 20)
        {
            if(timeUsed < cycle / 30 && currentRate < userRate)
            {
                currentRate = currentRate * 2;
            }
            Sleep(cycle - timeUsed);
            timeIndex += cycle * currentRate;
            emit signalSenter ->timeLapse(timeIndex);
        }
        else
        {
            currentRate = currentRate / 2;
            emit signalSenter ->slowDown(currentRate);
        }
        start = clock();
    }
}
